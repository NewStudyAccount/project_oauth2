  ...
  <div>
    ...
    <el-table :data="clients" border stripe>
      ...
      <el-table-column label="Scopes">
        <template #default="{ row }">
          <el-tag v-for="s in row.scopes" :key="s" type="info" size="small" style="margin-right: 4px">{{ s }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="Access Token 有效期" width="130">
        <template #default="{ row }">
          {{ formatTtl(row.accessTokenTtl) }}
        </template>
      </el-table-column>
      <el-table-column label="PKCE" width="70" align="center">
        <template #default="{ row }">
          <el-tag :type="row.requireProofKey ? 'success' : 'info'" size="small">{{ row.requireProofKey ? '是' : '否' }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column label="授权确认" width="90" align="center">
        <template #default="{ row }">
          <el-tag :type="row.requireAuthorizationConsent ? 'success' : 'info'" size="small">{{ row.requireAuthorizationConsent ? '是' : '否' }}</el-tag>
        </template>
      </el-table-column>
