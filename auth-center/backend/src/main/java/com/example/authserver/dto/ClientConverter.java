...
public final class ClientConverter {
    ...
    public static ClientDTO toDTO(RegisteredClient client) {
        ...
        dto.setIdTokenSignatureAlgorithm(tokenSettings.getIdTokenSignatureAlgorithm() != null
                ? tokenSettings.getIdTokenSignatureAlgorithm().getName() : "RS256");
        dto.setAccessTokenFormat(tokenSettings.getAccessTokenFormat() != null
                ? tokenSettings.getAccessTokenFormat().getValue() : "self-contained");
    ...
    public static RegisteredClient toEntity(ClientDTO dto, PasswordEncoderWrapper passwordEncoderWrapper) {
        ...
        if (dto.getClientSecretExpiresAt() != null) {
            builder.clientSecretExpiresAt(dto.getClientSecretExpiresAt());
        }
        ...
        // idTokenSignatureAlgorithm (仅支持 RS/ES 非对称签名算法)
        String algo = dto.getIdTokenSignatureAlgorithm() != null ? dto.getIdTokenSignatureAlgorithm() : "RS256";
        tokenSettingsBuilder.idTokenSignatureAlgorithm(SignatureAlgorithm.from(algo) != null
                ? SignatureAlgorithm.from(algo) : SignatureAlgorithm.RS256);

        // accessTokenFormat
        if ("reference".equals(dto.getAccessTokenFormat())) {
    ...
    public static RegisteredClient toEntityForUpdate(ClientDTO dto, RegisteredClient existing,
        ...
        merged.setClientSecretExpiresAt(dto.getClientSecretExpiresAt());
        merged.setReuseRefreshTokens(dto.isReuseRefreshTokens());
        merged.setIdTokenSignatureAlgorithm(dto.getIdTokenSignatureAlgorithm() != null ? dto.getIdTokenSignatureAlgorithm() : "RS256");
        merged.setAccessTokenFormat(dto.getAccessTokenFormat() != null ? dto.getAccessTokenFormat() : "self-contained");
